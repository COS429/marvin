    ...
    void shuffle(){
        ...
        for(int i =0; i <dataCPU.size();i++){
            ...
            if (bbInds[0] != i && bbInds[1] != i) {
                ...
                int eleLength = dataCPU[i]->dim[1];
                // Update bounding box indices according to the shuffled indices
                for(int m = 0; m < dataCPU[i]->dim[0]; m++) {
		  dataCPU[i]->CPUmem[m*eleLength] = CPUCompute2StorageT((float)(v_inv[(int)(CPUStorage2ComputeT(dataCPU[i]->CPUmem[m*eleLength]))]));
                }
            }
        }
